...
export const MultiStepForm = () => {
  const [formData, setFormData] = useState({ name: "", costume: "", strength: "" })
    ...
    <div className="multistep-form-container">
      ...
      {!formSubmitted && (
        <div className="nav-buttons">
          ...
          {currentStep > 1 && <button aria-label="go to the previous step" className="button" onClick={prevStep}>
            Back
          </button>}

          {currentStep < 3 ? (
            <button aria-label="go to the next step" className="button" onClick={nextStep}>
              Next
            </button>
          ) : (
            <button aria-label="submit your anwers" className="button" onClick={submitForm}>
              Submit
            </button>
          )}
        </div>
      ...
      {formSubmitted && (
        <div>
          <h3>
            All Set! Now, Dream of Your Heroic Adventures
          </h3>
          <p className="text-conclusion">
            Congratulations, you are officially the superheroe {formData.name} – fully equipped with a {formData.costume} costume, and the {formData.strength} superpower!
          </p>
          <p className="text-conclusion">
            Now close your eyes, drift off, and let the dreams of superpower shenanigans begin. 😴
          </p>
          <p>
            Sweet dreams, hero! 🌟
          </p>
